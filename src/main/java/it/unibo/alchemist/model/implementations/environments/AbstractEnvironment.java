...
import it.unibo.NeighbourhoodObserver;
import it.unibo.NeighbourhoodObserverEvent;
...
import static it.unibo.SimulationUtils.*;
...
public abstract class AbstractEnvironment<T, P extends Position<P>> implements Environment<T, P> {
    ...
    public final Set<String> observedMolecules = new HashSet<>();

    private final ArrayList<NeighbourhoodObserver> observers = new ArrayList<>();

    public void addObserver(NeighbourhoodObserver no){
        observers.add(no);
    }

    public <T> void notifyObservers(NeighbourhoodObserverEvent ev){
    ...
    private Queue<Operation> recursiveOperation(final Node<T> origin, final Node<T> destination, final boolean isAdd) {
        ...
        notifyObservers(new NeighbourhoodObserverEvent.NeighbourhoodChange(this, origin, destination, isAdd));
    ...
    protected final void updateNeighborhood(final Node<T> node, final boolean isNewNode) {
        ...
        if (Objects.requireNonNull(rule).isLocallyConsistent()) {
            ...
            // onNbrUpdate(newNeighborhood, oldNeighborhood, node, this);
            /*
            ...
            if (oldNeighborhood != null) {
                StreamSupport.stream(oldNeighborhood.spliterator(), false)
                        ...
                        .forEachOrdered(neighborhoodToChange -> {
                            ...
                            notifyObservers(new NeighbourhoodObserverEvent.NeighbourhoodChange(this, node, formerNeighbor, false));
                            if (!isNewNode) {
            ...
            for (final Node<T> newNeighbor: Sets.difference(newNeighborhood.getNeighbors(),
                ...
                notifyObservers(new NeighbourhoodObserverEvent.NeighbourhoodChange(this, node, newNeighbor, true));
                if (!isNewNode) {
